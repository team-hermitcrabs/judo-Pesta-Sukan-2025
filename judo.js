...
const SYNC_STATUS_ID_COLUMN = 6; // Column M for storing sync status
const SYNC_MESSAGE_ID_COLUMN = 7; // Column N for storing sync message 
...
function getAllSheetData(sheet, lastRow) {
  ...
  values.forEach((row, index) => {
    ...
    const isCompletelyEmpty = !row[0] && !row[1] && !row[2] && !row[3] && 
                             !row[4] && !row[5] && !row[6] && !row[7] && !hubdbRowId;
...
function findRowsNeedingHubDBCreation(allData, sheet, changes) {
  ...
  changedUniqueIds.forEach(uniqueId => {
    ...
    if (!data) return;
    const hasRequiredData = data.date_and_time && data.participant
    ...
    for (const rowInfo of rowsNeedingCreation) {  // Changed to for-of loop
      ...
      const payload = {
        ...
        data: {
            ...
            participant: rowInfo.data.participant,
            position: rowInfo.data.position,
        },
        metadata: {
...
function detectChanges(currentData, storedData) {
  ...
  Object.keys(currentData).forEach(uniqueId => {
    ...
    if (!stored) {
      ...
      const changedFields = [];
      const fieldsToCheck = ["date_and_time", "category", "participant", "position"];
